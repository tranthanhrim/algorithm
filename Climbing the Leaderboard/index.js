function findLowestRank(scores, length) {
  let rank = 1;
...
function climbingLeaderboard(scores, alice) {
  ...
  let anchorPoint = scoreLength - 1;
  const resultObj = {};
  let currentRank = findLowestRank(scores, scoreLength);
  let isAlwaysTop1 = false;
  const result = alice.map((aliceScore) => {
    if (isAlwaysTop1) return 1;
    if (resultObj[aliceScore]) return resultObj[aliceScore];
    let rank = -1;
    for(let i = anchorPoint; i > 0; i--) {
      ...
      if (scores[i] !== scores[i - 1]) currentRank--;
    }
    if (rank === -1) {
      if (scores[0] <= aliceScore) rank = 1;
      else rank = 2;
    }
    if (rank === -1) {
      rank = 1;
...
function readResultFile(resultCompare) {
  const length = resultCompare.length;
  const expectedResult = Array(length);
  let iLine = 0;
  lineReader.eachLine('expectedResult', function(line, last) {
    expectedResult[iLine++] = Number(line);
    if(last) {
      console.log('expectedResult: ', expectedResult.toString());
      console.log('resultCompare: ', resultCompare.toString());
      for(let i = 0; i < length; i++) {
        if (expectedResult[i] !== resultCompare[i]) console.log(i, expectedResult[i], resultCompare[i]);
      }
    }
  });
}
